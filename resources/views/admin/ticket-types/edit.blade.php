        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.ticket-types.update', $ticketType) }}" class="max-w-md">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <x-input-label for="name" :value="__('Ticket Type Name')" />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="price" class="block mt-1 w-full" type="number" name="price" step="0.01" min="0" :value="old('price', $ticketType->price)" required />
                        ...
                        <div class="mb-4">
                            ...
                            <select id="transport_type_id" name="transport_type_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach($transportTypes as $type)
                                    <option value="{{ $type->id }}" {{ (old('transport_type_id', $ticketType->transport_type_id) == $type->id) ? 'selected' : '' }}>
                                        {{ $type->name }}
                                    </option>
                                @endforeach
                            </select>
